...
#define CIRCLE_LEDS 40
#define NUM_LEDS CIRCLE_LEDS + 1
...
void loop() {
  ...
  if (writeTimer.poll(LED_INTERVAL)) {
    ...
    // copy the value from the end (buffered by 1) back to the start of the loop
    leds[0] = leds[NUM_LEDS];
    // TODO mix in values from sensor rather than overwrite 
